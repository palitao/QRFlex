...
import { LayoutDashboard, QrCode, BarChart3, CreditCard, Code, FileText, Menu, X, Bell, Settings, HelpCircle, LogOut } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex font-sans text-slate-900">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-white border-r border-slate-200 fixed h-full z-10">
        <div className="p-6 border-b border-slate-100 flex items-center justify-start gap-2">
          <img 
            src="https://i.imgur.com/8QhW5gS.png" 
            alt="QRFlex Logo" 
            className="h-8 w-auto object-contain" 
          />
          <span className="font-bold text-lg text-slate-900 tracking-tight">QRFlex</span>
        </div>
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          ...
          <SidebarItem icon={Settings} label="Configurações" id="settings" active={activePage === 'settings'} onClick={onNavigate} />
          <SidebarItem icon={HelpCircle} label="Suporte" id="support" active={activePage === 'support'} onClick={onNavigate} />
          <SidebarItem icon={FileText} label="Arquitetura Docs" id="docs" active={activePage === 'docs'} onClick={onNavigate} />
        </nav>

        <div className="p-4 border-t border-slate-100">
          <div className="flex items-center gap-3 px-2 mb-3">
            <img src={currentUser.foto_perfil} alt="User" className="w-9 h-9 rounded-full bg-slate-200 border border-slate-200" />
          ...
          <button 
            onClick={onLogout}
            className="w-full flex items-center justify-center gap-2 text-xs font-medium text-rose-600 bg-rose-50 hover:bg-rose-100 py-2 rounded-lg transition-colors"
          >
            <LogOut size={14} /> Sair
          </button>
        </div>
      ...
      <main className="flex-1 md:ml-64 flex flex-col min-h-screen transition-all duration-300">
        ...
        <header className="bg-white/80 backdrop-blur-md sticky top-0 z-20 border-b border-slate-200 h-16 px-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={() => onNavigate('qrcodes')} className="bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition-colors shadow-sm shadow-blue-200">
              + Novo QR Code
            </button>
          </div>
        </header>
        ...
        <div className="fixed inset-0 z-50 md:hidden flex">
          ...
          <div className="relative w-64 bg-white h-full shadow-xl flex flex-col">
            ...
            <nav className="p-4 space-y-1 flex-1 overflow-y-auto">
              ...
              <SidebarItem icon={Settings} label="Configurações" id="settings" active={activePage === 'settings'} onClick={(p: string) => { onNavigate(p); setMobileMenuOpen(false); }} />
              <SidebarItem icon={HelpCircle} label="Suporte" id="support" active={activePage === 'support'} onClick={(p: string) => { onNavigate(p); setMobileMenuOpen(false); }} />
            </nav>
            <div className="p-4 border-t border-slate-100">
               <button onClick={onLogout} className="w-full py-2 bg-rose-50 text-rose-600 rounded-lg text-sm font-bold">Sair</button>
            </div>
          </div>
        </div>
      )}
